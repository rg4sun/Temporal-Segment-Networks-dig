...
print('Pkgs importing successfully!')


CKPT_PATH = '/data2/hdu/shh/ckpt-station/rgb-280.ckpt' # discarded, now ctrled by run_test.sh
...
parser.add_argument('--weights', type=str, default=CKPT_PATH)
...
parser.add_argument('--test_list', type=str, default="/data2/hdu/shh/data/data_extracted/ucf101/ucf101_val_split_1_rawframes.txt")
# parser.add_argument('--dataset_path', type=str, default="/data/Fancyshun/TSN/dataset/tvl1/")
parser.add_argument('--dataset_path', type=str, default="/data2/hdu/shh/data/data_extracted/ucf101/tvl1")

parser.add_argument('--device_id', type=int, default=1)

parser.add_argument('--arch', type=str, default="BNInception")
# parser.add_argument('--save_scores', type=str, default="./score/score_warmup", help="./score/score_warmup_flow_2_")
parser.add_argument('--save_scores', type=str, default="/data2/hdu/shh/rgb-score", help="./score/score_warmup_rgb_2_")
...
print('====== load {} successfully ======'.format(args.weights))
...
# 增加提示
print('='*100)
print('args.save_scores = {}'.format(args.save_scores))
print('args.save_scores is not None = {}'.format(args.save_scores is not None))
if args.save_scores is not None:

    # 检查保存路径是否存在，之前一直保存不了是因为路径不存在
    if not os.path.exists(args.save_scores):
        print('Creating <{}> for saving scores...'.format(args.save_scores))
        os.mkdir(args.save_scores)
    ...
    for i in range(len(output)): # output 就是预测的结果list
        idx = order_dict[name_list[i]]
    ...
    # 增加提示
    print('+' * 100)
    score_file_path = os.path.join(args.save_scores, '{}_scores.npz'.format(args.modality))
    print(' score file saved as {}'.format(score_file_path))
    # print('-' * 100)
    # print('scores=reorder_output # reorder_output = {}'.format(reorder_output))
    # print('-'*100)
    # print('labels=reorder_label # reorder_label = {}'.format(reorder_label))
    print('*'*100)
    print('type of reorder_output and reorder_label: {} and {}'.format(type(reorder_output), type(reorder_label)))
    print('-'*50, ' Starting score file saving via np.savez()', '-'*50)

    # np.savez(args.save_scores+args.modality, scores=reorder_output, labels=reorder_label)
    # np.savez(args.save_scores + args.modality + '.npz', scores=reorder_output, labels=reorder_label)

    # np.savez('/data2/hdu/shh/rgb-score/rgb_scores.npz', scores=reorder_output, labels=reorder_label)
    np.savez(score_file_path, scores=reorder_output, labels=reorder_label)
    print('*' * 50, '{} saved successfully !',format(score_file_path), '*' * 50)

    # print('+'*50, 'np.savetxt() testing ...', '+'*50)
    # print('Saving labels_output.txt...')
    # np.savetxt('/data2/hdu/shh/rgb-score/labels_output.txt', reorder_label)
    # print('/data2/hdu/shh/rgb-score/labels_output.txt saved!')

    # print('+'*50, 'np.savetxt() testing ...', '+'*50)
    # print('Saving labels_output.txt...')
    # np.savetxt('/data2/hdu/shh/rgb-score/scores_output.txt', reorder_output)
    # print('/data2/hdu/shh/rgb-score/scores_output.txt saved!')

    # print('+'*50, 'np.save() testing ...', '+'*50)
    # print('Saving labels_output.npy...')
    # np.save('/data2/hdu/shh/rgb-score/labels_output.npy', reorder_label)
    # print('/data2/hdu/shh/rgb-score/labels_output.npy saved!')
    #
    # print('Saving scores_output.npy...')
    # np.save('/data2/hdu/shh/rgb-score/scores_output.npy', reorder_output)
    # print('/data2/hdu/shh/rgb-score/scores_output.npy saved!')
