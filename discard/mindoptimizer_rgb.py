...
def path_attacher(path):
    return os.path.join(ROOT_PATH[PLATFORM], path)

PLATFORM = 0 # 0 huawei, 1 sccas
ROOT_PATH = ['/data2/hdu/shh', '/home/shh_ucf/']
CKPT_PATH =  os.path.join(ROOT_PATH[PLATFORM], 'ckpt-records/rgb_checkpoint-40/best.ckpt') # 160+40
EPOCH = 100 # distrubute脚本会弃用
GPU_ID = 2
...
parser.add_argument('--dataset_path', type=str, default=path_attacher('data/data_extracted/ucf101/tvl1'))
...
parser.add_argument('--train_list', type=str, default=path_attacher("data/data_extracted/ucf101/ucf101_train_split_1_rawframes.txt"))
# parser.add_argument('--val_list', type=str, default="/data/Fancyshun/TSN/dataset/ucf101_val_split_1_rawframes.txt")
parser.add_argument('--val_list', type=str, default=path_attacher("data/data_extracted/ucf101/ucf101_val_split_1_rawframes.txt"))
# parser.add_argument('--pretrained_path', type=str, default="/data/Fancyshun/TSN/tsn-pytorch/bninception_flow.npy")
# 增加一个预训练判断
parser.add_argument('--pretrained', type=bool, default=True) # RGB 模式第一次跑没有预训练文件
parser.add_argument('--pretrained_path', type=str, default=CKPT_PATH) # 第三次训练
# parser.add_argument('--pretrained_path', type=str, default=None) # RGB 模式没有预训练文件
parser.add_argument('--device_id', default=GPU_ID, type=int)
...
# ========================= Learning Configs ==========================
parser.add_argument('--epochs', default=EPOCH, type=int, help='number of total epochs to run') # 每次跑到80就结束了
...
parser.add_argument('--eval_start_epoch', default=1, type=int, help='')
...
# parser.add_argument('--ckpt_save_dir', type=str, default="/home/rgb_checkpoint")
parser.add_argument('--ckpt_save_dir', type=str, default=path_attacher("rgb_checkpoint"))
...
if __name__ == '__main__':
    ...
    if args.save_check_point and (device_num == 1 or device_id == 0): # 单卡的时候不一定device_id == 0，不过不影响，因为device_num == 1
    ...
    ds.config.set_enable_shared_mem(False) # import mindspore.dataset as ds
    # 这里正式跑训练
